...
def test_destroy_environment_plan_only_does_not_clear_tracked_packages(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
...
def test_destroy_environment_execute_clears_tracked_packages_after_successful_rm(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
    ...
    loaded_manifest = environment_registry.load("ubuntu-24.04-default")
    output = capsys.readouterr().out
    assert exit_code == 0
    assert loaded_manifest.status == "defined"
    assert loaded_manifest.installed_packages() == []
    assert "Cleared runtime package tracking." in output
...
def test_destroy_environment_rm_failure_leaves_tracked_packages(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
...
def test_destroy_environment_manifest_update_failure_leaves_tracking_and_warns(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
...
def test_destroy_environment_global_json_execute_output(
    ...
    assert output == {
        ...
        "status": "defined",
        "executed": True,
        "cleared_tracked_packages": False,
        "actions": [
            {
                "id": "remove-container",
                "description": "Remove managed environment container",
...
def test_destroy_environment_subcommand_json_execute_output(
    ...
    assert output["executed"] is True
    assert output["cleared_tracked_packages"] is False
    assert output["actions"][0]["command"] == ["podman", "rm", "appresolver-env-ubuntu-24.04-default"]


def test_destroy_environment_json_execute_output_reports_cleared_tracked_packages(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
    ...
    output = json.loads(capsys.readouterr().out)
    assert exit_code == 0
    assert output["cleared_tracked_packages"] is True
...
def test_reconcile_environment_already_consistent_does_not_update(
    ...
    assert exit_code == 0
    assert "No reconciliation needed." in capsys.readouterr().out
    assert environment_registry.load("ubuntu-24.04-default").status == "running"


def test_reconcile_environment_consistent_human_output_has_no_execute_guidance(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="defined")
    ...
    output = capsys.readouterr().out
    assert exit_code == 0
    assert "No reconciliation needed." in output
    assert "Execution not performed. Re-run with --execute to update the manifest." not in output
    assert environment_registry.load("ubuntu-24.04-default").status == "defined"


def test_reconcile_environment_defined_missing_no_such_object_execute_does_not_update(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="defined")
...
def test_reconcile_environment_consistent_json_output_reports_not_executed(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="defined")
...
def test_install_package_plan_only_prints_actions_without_subprocess_or_mutation(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
    ...
    assert "Planned Podman actions for ubuntu-24.04-default:" in output
    assert "podman start appresolver-env-ubuntu-24.04-default" in output
    assert "podman exec appresolver-env-ubuntu-24.04-default apt-get update" in output
    assert "Execution not performed. Re-run with --execute to install the package." in output
    assert environment_registry.load("ubuntu-24.04-default").status == "created"


@pytest.mark.parametrize("status", ["created", "stopped"])
def test_install_package_execute_starts_non_running_environment_then_installs_and_marks_running(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, status: str
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status=status)
    calls: list[list[str]] = []
...
def test_install_package_execute_running_environment_skips_start_and_does_not_change_status(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
...
def test_install_package_start_failure_prevents_apt_commands(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
...
def test_install_package_apt_update_failure_prevents_install(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="running")
    ...
    assert exit_code == 1
    assert calls == [["podman", "exec", "appresolver-env-ubuntu-24.04-default", "apt-get", "update"]]


def test_install_package_apt_install_failure_exits_nonzero(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="running")
...
def test_install_package_manifest_update_failure_after_start_warns_and_skips_apt(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
    ...
    assert exit_code == 1
    assert calls == [["podman", "start", "appresolver-env-ubuntu-24.04-default"]]
    assert "runtime may be running but registry update failed" in stderr
    assert environment_registry.load("ubuntu-24.04-default").status == "created"


@pytest.mark.parametrize("package_name", ["bad package", "bad;package", "bad/package", "$(bad)", "`bad`"])
def test_install_package_rejects_invalid_package_without_subprocess(
    ...
    def fail_run_command(command: list[str]) -> subprocess.CompletedProcess[str]:
        raise AssertionError("invalid package must not call subprocess")

    monkeypatch.setattr(podman, "run_command", fail_run_command)

    exit_code = main(["--registry-dir", str(registry_dir), "install-package", "ubuntu-24.04-default", package_name])

    assert exit_code == 1


def test_install_package_invalid_status_does_not_call_subprocess(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="defined")
    ...
    def fail_run_command(command: list[str]) -> subprocess.CompletedProcess[str]:
        raise AssertionError("invalid status must not call subprocess")
...
def test_install_package_missing_environment_exits_nonzero(tmp_path: Path) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"

    exit_code = main(["--registry-dir", str(registry_dir), "install-package", "missing-env", "curl"])

    assert exit_code == 1


def test_install_package_non_container_backend_exits_nonzero_without_subprocess(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, environment_id="flatpak-env", backend="flatpak", image="runtime")
    ...
    def fail_run_command(command: list[str]) -> subprocess.CompletedProcess[str]:
        raise AssertionError("non-container backend must not call subprocess")

    monkeypatch.setattr(podman, "run_command", fail_run_command)

    exit_code = main(["--registry-dir", str(registry_dir), "install-package", "flatpak-env", "curl"])

    assert exit_code == 1


def test_install_package_unsupported_image_exits_nonzero_without_subprocess(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, image="fedora:latest", status="running")

    def fail_run_command(command: list[str]) -> subprocess.CompletedProcess[str]:
        raise AssertionError("unsupported image must not call subprocess")
...
def test_global_json_install_package_plan_outputs_structured_plan(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="running")
    ...
    def fail_run_command(command: list[str]) -> subprocess.CompletedProcess[str]:
        raise AssertionError("plan-only JSON must not call subprocess")

    monkeypatch.setattr(podman, "run_command", fail_run_command)

    exit_code = main(
        ["--registry-dir", str(registry_dir), "--json", "install-package", "ubuntu-24.04-default", "curl"]
    )
    ...
    assert output["status"] == "planned-install"
    assert output["executed"] is False
    assert [action["id"] for action in output["actions"]] == ["apt-update", "apt-install"]


def test_subcommand_json_install_package_plan_outputs_structured_plan(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="running")
    ...
    exit_code = main(
        ["--registry-dir", str(registry_dir), "install-package", "ubuntu-24.04-default", "curl", "--json"]
    )
...
def test_global_json_install_package_execute_outputs_structured_success(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="running")
...
def test_subcommand_json_install_package_execute_outputs_structured_success(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="running")
...
def test_show_environment_packages_empty_after_successful_destroy(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
    ...
    main(["--registry-dir", str(registry_dir), "destroy-environment", "ubuntu-24.04-default", "--execute"])
    capsys.readouterr()
    exit_code = main(["--registry-dir", str(registry_dir), "show-environment-packages", "ubuntu-24.04-default"])
...
def test_remove_package_plan_only_does_not_call_subprocess_or_mutate_tracking(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
    ...
    def fail_run_command(command: list[str]) -> subprocess.CompletedProcess[str]:
        raise AssertionError("plan-only remove-package must not call subprocess")

    monkeypatch.setattr(podman, "run_command", fail_run_command)

    exit_code = main(["--registry-dir", str(registry_dir), "remove-package", "ubuntu-24.04-default", "curl"])

    output = capsys.readouterr().out
    assert exit_code == 0
    assert "podman exec appresolver-env-ubuntu-24.04-default apt-get remove -y curl" in output
    assert "Execution not performed. Re-run with --execute to remove the package." in output
    assert environment_registry.load("ubuntu-24.04-default").installed_packages() == [
        {"name": "curl", "manager": "apt", "installed_at": "2026-06-03T12:00:00+00:00"}
    ]


def test_remove_package_requires_tracked_package_without_subprocess(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="running")
    ...
    def fail_run_command(command: list[str]) -> subprocess.CompletedProcess[str]:
        raise AssertionError("untracked remove-package must not call subprocess")
...
def test_remove_package_validates_package_name_without_subprocess(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
    ...
    def fail_run_command(command: list[str]) -> subprocess.CompletedProcess[str]:
        raise AssertionError("invalid remove-package must not call subprocess")

    monkeypatch.setattr(podman, "run_command", fail_run_command)

    exit_code = main(["--registry-dir", str(registry_dir), "remove-package", "ubuntu-24.04-default", "bad/name"])

    assert exit_code == 1


@pytest.mark.parametrize("status", ["created", "stopped"])
def test_remove_package_execute_starts_non_running_environment_then_removes_and_untracks(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, status: str
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status=status)
    ...
    exit_code = main(
        ["--registry-dir", str(registry_dir), "remove-package", "ubuntu-24.04-default", "curl", "--execute"]
    )
    ...
    assert calls == [
        ["podman", "start", "appresolver-env-ubuntu-24.04-default"],
        ["podman", "exec", "appresolver-env-ubuntu-24.04-default", "apt-get", "remove", "-y", "curl"],
    ]
    assert loaded_manifest.status == "running"
    assert loaded_manifest.installed_packages() == []


def test_remove_package_execute_running_environment_skips_start_and_untracks(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
    ...
    assert environment_registry.load("ubuntu-24.04-default").installed_packages() == []
    assert "Removed and untracked package curl from environment ubuntu-24.04-default" in output


def test_remove_package_start_failure_prevents_apt_remove(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
...
def test_remove_package_apt_remove_failure_leaves_tracking(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
...
def test_remove_package_tracking_update_failure_after_apt_remove_warns_runtime_registry_divergence(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
...
def test_global_json_remove_package_plan_outputs_structured_plan(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
    ...
    def fail_run_command(command: list[str]) -> subprocess.CompletedProcess[str]:
        raise AssertionError("plan-only JSON remove-package must not call subprocess")

    monkeypatch.setattr(podman, "run_command", fail_run_command)

    exit_code = main(
        ["--registry-dir", str(registry_dir), "--json", "remove-package", "ubuntu-24.04-default", "curl"]
    )
    ...
    assert output["package"] == "curl"
    assert output["package_manager"] == "apt"
    assert output["status"] == "planned-remove"
    assert output["executed"] is False
    assert [action["id"] for action in output["actions"]] == ["apt-remove"]


def test_subcommand_json_remove_package_execute_outputs_structured_success(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
    ...
    assert output["executed"] is True
    assert output["tracked"] is False
    assert [action["id"] for action in output["actions"]] == ["apt-remove"]


def test_environment_summary_reports_defined_missing_state(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="defined")
    ...
    exit_code = main(["--registry-dir", str(registry_dir), "environment-summary", "ubuntu-24.04-default"])

    output = capsys.readouterr().out
    assert exit_code == 0
    assert "Runtime status: missing" in output
    assert "Consistent: true" in output
    assert "create-environment" in output


def test_environment_summary_reports_running_runtime_with_tracked_packages(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
    ...
    exit_code = main(["--registry-dir", str(registry_dir), "--json", "environment-summary", "ubuntu-24.04-default"])

    assert exit_code == 0


def test_environment_summary_json_running_runtime_shape(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="running")
    ...
    assert output == {
        ...
        "available_actions": ["stop-environment", "install-package", "remove-package"],
    }


def test_environment_summary_reports_inconsistent_state_and_suggests_reconcile(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="created")
    ...
    exit_code = main(["--registry-dir", str(registry_dir), "environment-summary", "ubuntu-24.04-default", "--json"])

    output = json.loads(capsys.readouterr().out)
    assert exit_code == 0
    assert output["consistent"] is False
    assert output["suggested_status"] == "defined"
    assert output["available_actions"] == ["reconcile-environment"]


def test_environment_summary_reports_created_stopped_runtime_actions(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="created")
    ...
    output = json.loads(capsys.readouterr().out)
    assert exit_code == 0
    assert output["runtime_status"] == "stopped"
...
def test_environment_summary_does_not_mutate_manifest(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    environment_registry = save_environment_manifest(registry_dir, status="created")
...
def test_environment_summary_subcommand_json_works(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    registry_dir = tmp_path / ".appresolver" / "apps"
    save_environment_manifest(registry_dir, status="defined")
    ...
    output = json.loads(capsys.readouterr().out)
    assert exit_code == 0
    assert output["available_actions"] == ["create-environment"]
